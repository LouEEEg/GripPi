...
#define Joint3Limit 14 //Joint 3 limit switch
#define Joint4Limit 19  //Joint 4 limit switch
#define enablepin 40
...
int state = 0;
int PiInput = 0;
volatile static float motor_speed = 0;

//defining all the motors
//Connect PWM pin to DIR- and connect Direction pin to PUL-
AccelStepper BaseStepper(AccelStepper::DRIVER, 2, 7);  //BaseStepper is connected to pins 7(PWM), 2(direction), 3(interupt)
//AccelStepper JointStepper1(AccelStepper::DRIVER, 5, 6); //Joint1 is connected to pins 6(PWM), 5(direction), 4(interupt)
//AccelStepper JointStepper2(AccelStepper::DRIVER, 10, 9); //Joint2 is connected to pins 9(PWM), 10(Direction), 11(Interupt)
//AccelStepper JointStepper3(AccelStepper::DRIVER,12,13);//Joint3 is connected to pins 13(PWM), 12(Direction), 14(interupt)
//AccelStepper JointStepper4(AccelStepper::DRIVER,18,17);//Joint4 is connected to pins 17(Pwm), 18(direction), 19(Interupt)
...
void setup() 
{
  ...
  digitalWrite(enablepin, LOW);
   ...
   //motor direction and speed(towards the limit switch)
  /* 
