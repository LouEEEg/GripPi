...
void setup() {
  ...
  Serial.println("This demo expects 4 pieces of data - an X, Y, Z and grip values");
  Serial.println("Enter data in this style <X value, Y value, Z value Grip Value (90 open, 130 closed)>  ");
  Serial.println();
...
// ------------------------------------- //
  ...
  void CalculateTrajectory(Manipulator GripPi){
    ...
    //
    float xt=GripPi.goal.x -0; //previous linked list x position ; //difference in the x,y,z values from the current linked list position to the goal 
    float yt=GripPi.goal.y -0; //previous linked list y position; 
    float zt=GripPi.goal.z -0; //previous linked list z position; 
    GripPi.goal.distance=sqrt(sq(xt)+sq(yt)+sq(zt));//total distance from the current position to the goal 
    float D = 10; //incrimental distance in mm we want each point interval to travel 
    ...
    for( D < GripPi.goal.distance; D >= GripPi.goal.distance;){
      ...
      // change these to add to the previous linked list entry 
...
// ---------------------------- //
  void RxGoalFromSerial(SerialInput GripPi_Serial, Manipulator GripPi) {
    ...
    while (Serial.available() > 0 && GripPi_Serial.new_data == false) {
    ...
    if (GripPi_Serial.new_data == true) {
      ...
      if(GripPi_Serial.Rx_x==999){
        WakeUp();
        Serial.print("GripPi is Calibrated");
        GripPi.goal.x=800;
        GripPi_Serial.new_data = false;
        }
      else if(GripPi_Serial.Rx_x==998){
        ReCalibrate();
        GripPi_Serial.new_data = false;
      }
    /*
      else if(GripPi.goal.x == 485 && GripPi.goal.y == 220 && GripPi.goal.z == 315){//bin 1 position 
        //start linked list with a safe point in front of bin 1 
        CalculateTrajectory(GripPi);
        //linked list 
        GripPi_Serial.new_data = false;
      }

      else if(GripPi.goal.x == 505 && GripPi.goal.y == 220 && GripPi.goal.z ==150){//bin 2 position 
        //start linked list with a safe point in front of bin 2
        CalculateTrajectory( GripPi);        
        GripPi_Serial.new_data = false;
      }

      else if(GripPi.goal.x == 485 && GripPi.goal.y == 220 && GripPi.goal.z ==0){ //bin 3 position 
        //start linked list with a safe point in front of bin 3
        CalculateTrajectory( GripPi);
        GripPi_Serial.new_data = false;
      }

      else if(GripPi.goal.x == 5 && GripPi.goal.y == 120 && GripPi.goal.z ==345){ //user location 
        //start linked list with a safe point in front of bin 1 
        CalculateTrajectory( GripPi);
        GripPi_Serial.new_data = false;
      }
  */
  ...
  void printParsedData(SerialInput GripPi_serial) {
    Serial.print("X ");
    Serial.println(GripPi.goal.x);
    Serial.print("Y ");
    Serial.println(GripPi.goal.y);
    Serial.print("Z ");
    Serial.println(GripPi.goal.z);
    Serial.print("Grip ");
